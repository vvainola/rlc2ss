...
std::unique_ptr<Model_converter::StateSpaceMatrices> calcStateSpace(Eigen::MatrixXd const& K1,
...
Model_converter::StateSpaceMatrices const& calcStateSpaceMatrices(Model_converter::Components const& components,
...
Model_converter::Outputs calcInstantaneousOutputs(Model_converter::Components const& components,
                                                  Model_converter::States const& states,
                                                  Model_converter::Inputs const& inputs,
                                                  uint64_t switch_combination) {
    Model_converter::Outputs instantaneous_outputs;
...
uint64_t externalClosedSwitchMask(Model_converter::Switches const& switches) {
...
bool diodeSolverClosed(Model_converter::Switches const& switches, size_t diode_idx) {
...
uint64_t solverClosedDiodeMask(Model_converter::Switches const& switches) {
    uint64_t mask = 0;
...
uint64_t inductorCurrentSignMask(Model_converter::States const& states) {
    uint64_t mask = 0;
    if (states.I_L_a > 0.0) {
        mask |= uint64_t{1} << 0;
...
double diodeCurrent(size_t diode_idx, Model_converter::Outputs const& outputs) {
    switch (diode_idx) {
        case 0: return outputs.I_R_D_a_n;
        case 1: return outputs.I_R_D_a_p;
        case 2: return outputs.I_R_D_b_n;
        case 3: return outputs.I_R_D_b_p;
        case 4: return outputs.I_R_D_c_n;
        case 5: return outputs.I_R_D_c_p;
    default:
        return 0.0;
    }
}
...
double diodeForwardOverdrive(size_t diode_idx,
                             Model_converter::Outputs const& outputs,
                             Model_converter::Inputs const& inputs) {
    // Positive overdrive means an open diode would be forward-biased
    // for this instantaneous solution.
    switch (diode_idx) {
        case 0: return outputs.N_dc_n - outputs.N_c_a - inputs.V_D_a_n;
        case 1: return outputs.N_c_a - outputs.N_dc_p - inputs.V_D_a_p;
        case 2: return outputs.N_dc_n - outputs.N_c_b - inputs.V_D_b_n;
        case 3: return outputs.N_c_b - outputs.N_dc_p - inputs.V_D_b_p;
        case 4: return outputs.N_dc_n - outputs.N_c_c - inputs.V_D_c_n;
        case 5: return outputs.N_c_c - outputs.N_dc_p - inputs.V_D_c_p;
    default:
        return 0.0;
    }
}

double inductorCurrentDiscontinuity(Model_converter::Outputs const& outputs,
                                    Model_converter::States const& states) {
    ...
    discontinuity = std::max(discontinuity, std::abs(outputs.data[5] - states.data[0]));
    discontinuity = std::max(discontinuity, std::abs(outputs.data[6] - states.data[1]));
    discontinuity = std::max(discontinuity, std::abs(outputs.data[7] - states.data[2]));
    discontinuity = std::max(discontinuity, std::abs(outputs.data[8] - states.data[3]));
    discontinuity = std::max(discontinuity, std::abs(outputs.data[9] - states.data[4]));
    discontinuity = std::max(discontinuity, std::abs(outputs.data[10] - states.data[5]));
    return discontinuity;
}

Model_converter::Switches applySolverDiodeMask(Model_converter::Switches switches, uint64_t solver_closed_diode_mask) {
    ...
    switches.S_D_a_n.forceOutput((solver_closed_diode_mask & (uint64_t{1} << 0)) != 0 ? std::optional<bool>{true} : std::nullopt);
    switches.S_D_a_p.forceOutput((solver_closed_diode_mask & (uint64_t{1} << 1)) != 0 ? std::optional<bool>{true} : std::nullopt);
    switches.S_D_b_n.forceOutput((solver_closed_diode_mask & (uint64_t{1} << 2)) != 0 ? std::optional<bool>{true} : std::nullopt);
    switches.S_D_b_p.forceOutput((solver_closed_diode_mask & (uint64_t{1} << 3)) != 0 ? std::optional<bool>{true} : std::nullopt);
    switches.S_D_c_n.forceOutput((solver_closed_diode_mask & (uint64_t{1} << 4)) != 0 ? std::optional<bool>{true} : std::nullopt);
    switches.S_D_c_p.forceOutput((solver_closed_diode_mask & (uint64_t{1} << 5)) != 0 ? std::optional<bool>{true} : std::nullopt);
    return switches;
}

Model_converter::Switches releaseReverseCurrentDiodes(Model_converter::Components const& components,
                                                      Model_converter::States const& states,
                                                      Model_converter::Inputs const& inputs,
                                                      Model_converter::Switches const& switches) {
...
Model_converter::Switches resolveDiodeContinuity(Model_converter::Components const& components,
                                                 Model_converter::States const& states,
                                                 Model_converter::Inputs const& inputs,
                                                 Model_converter::Switches const& switches,
